#requires:
#terf:temp syringe_tank to be the tank
...
#tank data
execute store result score fluid_amount terf_states run data get storage terf:temp syringe_tank.amount
execute store result score fluid_max terf_states run data get storage terf:temp syringe_tank.max

##Fail
#fail if tank is empty
execute if score fluid_amount terf_states matches ..0 run return run playsound item.bucket.fill_powder_snow player @a[distance=0..] ~ ~ ~ 1 2
#fail if types dont match
data modify storage terf:temp args.type set from storage terf:temp syringe_tank.id
...
execute unless score succeeded terf_states matches 0 unless score syringe_amount terf_states matches 0 run return run playsound item.bucket.fill_tadpole player @a[distance=0..] ~ ~ ~ 1 1
#if changetype returns error
data modify storage terf:temp syringe_tank.new_id set from storage terf:temp syringe_tank.id
scoreboard players set error terf_states 0
function terf:entity/player/tool/syringe/run_changetype with storage terf:temp syringe_tank
execute unless score error terf_states matches 0 run return fail

##Transfer fluids
#clamp transfer amount
execute if score transfer_amount terf_states > fluid_amount terf_states run scoreboard players operation transfer_amount terf_states = fluid_amount terf_states
...
execute if score syringe_amount terf_states > syringe_capacity terf_states run scoreboard players operation fluid_amount terf_states += syringe_amount terf_states
execute if score syringe_amount terf_states > syringe_capacity terf_states run scoreboard players operation fluid_amount terf_states -= syringe_capacity terf_states
execute if score syringe_amount terf_states > syringe_capacity terf_states run scoreboard players operation syringe_amount terf_states = syringe_capacity terf_states

##Explode if antimatter
function terf:require/get_fluid_dictionary with storage terf:temp syringe_tank
...
##Save data
playsound item.bucket.fill_lava player @a[distance=0..] ~ ~ ~ 1 2
#tank data
execute store result storage terf:temp syringe_tank.amount int 1 run scoreboard players get fluid_amount terf_states
#syringe data
...
data modify entity 0010ccd2-0010-cd37-0010-cd360010c8e1 item.components.minecraft:custom_data.fluids[0].id set from storage terf:temp syringe_tank.id
