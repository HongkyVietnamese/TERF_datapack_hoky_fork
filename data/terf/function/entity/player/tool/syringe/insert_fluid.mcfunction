...
##Fail
#if tank is full
execute if score fluid_amount terf_states >= fluid_max terf_states run return run playsound item.bucket.empty_powder_snow player @a[distance=0..] ~ ~ ~ 1 2
#if types dont match
...
execute unless score succeeded terf_states matches 0 run return run playsound item.bucket.empty_tadpole player @a[distance=0..] ~ ~ ~ 1 1
...
execute if score transfer_amount terf_states > syringe_amount terf_states run scoreboard players operation transfer_amount terf_states = syringe_amount terf_states
...
##Save data
playsound item.bucket.empty_lava player @a[distance=0..] ~ ~ ~ 1 2
#tank data
